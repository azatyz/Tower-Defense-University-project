...
class Path:
    ...
    def is_position_on_path(self, x, y, tower_radius):
        """Проверить, пересекается ли позиция башни с путём (сложная функция - коллизия)"""
        # Проверяем расстояние до каждого отрезка пути
        for i in range(len(self.points) - 1):
            x1, y1 = self.points[i]
            x2, y2 = self.points[i + 1]
            
            # Вычисляем расстояние от точки (x, y) до линии отрезка
            distance = self._distance_to_segment(x, y, x1, y1, x2, y2)
            
            # Если расстояние меньше чем радиус башни + ширина пути - это пересечение
            if distance < tower_radius + self.path_width:
                return True
    ...
    def _distance_to_segment(x, y, x1, y1, x2, y2):
        """Вычислить расстояние от точки до линейного отрезка"""
        ...
        # Параметр t определяет позицию проекции на отрезок
...
def game_loop():
    ...
    enemies = []
    projectiles = []
    towers = [Tower(WIDTH - 150, HEIGHT // 2)]  # Начальная башня (может быть несколько)
    game_manager = GameManager()
    tower_cost = 150  # Стоимость башни
    ...
    while running:
        for event in pygame.event.get():
            ...
            # Клик мышкой - расстановка башни
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1 and not game_manager.game_over:  # Левая кнопка мышки, не в Game Over
                    mouse_x, mouse_y = event.pos
                    print(f"🖱️ Клик в ({mouse_x}, {mouse_y}) | Деньги: {game_manager.money}")
                    
                    # Проверяем, достаточно ли денег
                    if game_manager.money >= tower_cost:
                        print(f"💰 Денег достаточно!")
                        
                        # Проверяем, не пересекается ли с другими башнями
                        can_build = True
                        for existing_tower in towers:
                            distance = ((mouse_x - existing_tower.x) ** 2 + 
                                      (mouse_y - existing_tower.y) ** 2) ** 0.5
                            if distance < 80:  # Минимальное расстояние между башнями
                                can_build = False
                                print(f"⚠️ Слишком близко к башне! Расстояние: {distance}")
                                break
                        ...
                        if can_build:
                            ...
                            print(f"✅ Башня построена! Осталось денег: {game_manager.money}")
                    else:
                        print(f"❌ Недостаточно денег! Нужно {tower_cost}, есть {game_manager.money}")
        ...
        if not game_manager.game_over:
            ...
            # Обновляем все башни и их стрельбу
            for tower in towers:
                tower.update()
        ...
        # Инструкция по строительству башен
        build_text = f"Кликни для постройки башни (стоит {tower_cost}$) | Осталось башен в радиусе: {len(towers)}"
        build_surface = font.render(build_text, True, (200, 200, 100))
        screen.blit(build_surface, (10, 70))
